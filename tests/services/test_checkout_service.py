...
def test_create_checkout_session_with_expired_cart_items(db_session):
    logger.debug("Test checkout session with expired cart items")
    ...
    with pytest.raises(HTTPException) as exc_info:
        checkout_service.create_checkout_session(db_session, test_cart.session_id)

    cart_expired_items_removed = db_session.query(CartModel).filter(CartModel.id == test_cart.id).first()
    assert exc_info.value.status_code == status.HTTP_400_BAD_REQUEST
    assert exc_info.value.detail == "Some items expired. Please review your cart"
